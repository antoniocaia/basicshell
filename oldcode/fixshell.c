...
bool i_want_to_die(char *buffer, int ind)
{
	while (true)
	{
		...
		if (buffer[ind] == ' ' || is_char_in_list(buffer[ind], spec_chs, sizeof(spec_chs)) == 0)
			return false;
		ind++;
	}
}
...
char **parse_line(char *buffer)
{
	...
	while (buffer[bf_end] != '\0')
	{
		...
		else
		{
			// Standard command
			bf_end++;
			while (is_char_in_list(buffer[bf_end], spec_chs, sizeof(spec_chs)) == 1)
				bf_end++;
...
char **parse_command(char *buffer)
{
	...
	while (buffer[bf_end] != '\0')
	{
		...
		else
		{
			// Standard command
			bf_end++;
			while (is_char_in_list(buffer[bf_end], red_chs, sizeof(red_chs)) == 1)
			{
