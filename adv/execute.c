...
void update_exec_res(int status) {
  if (status != 0)
...
int execute(tok ** tokens) {
  ...
  // Check for io (TODO: fix "stop")
  int tks_ind = 0;
  while (tokens[tks_ind] != 0 && tokens[tks_ind] -> type != 3) {
  ...
  while (tokens[tks_ind] != 0) {
    if (tokens[tks_ind] -> type == 1 && can_ex) // COMMAND
    {
      prep_command(tokens, tks_ind);
      bang = update_bang(bang);
    } else if (tokens[tks_ind] -> type == 3) {
      can_ex = set_connector(tokens[tks_ind]); // CONNECTOR ; || &&
    } else if (strcmp(tokens[tks_ind] -> value, "(") == 0) // SUBSHELL ( )
    {
      ...
      bang = update_bang(bang);
      tks_ind;
    } else if (strcmp(tokens[tks_ind] -> value, "!") == 0) // BANG !
    {
      bang = true;
    }
